...
class TianmaGraphConvolution(Module):
    def __init__(self, in_features, out_features, bias=True):
        super(TianmaGraphConvolution, self).__init__()
...
class TianmaGraphEncoder(Module):
    def __init__(self, device, entity, emb_size, kg, embeddings=None, fix_emb=True, seq='rnn', gcn=True, hidden_size=100, layers=1, rnn_layer=1):
        super(TianmaGraphEncoder, self).__init__()
        ...
        if self.gcn:
            ...
            for _ in range(layers):
                self.gnns.append(TianmaGraphConvolution(indim, outdim))
                indim = outdim
        else:
    ...
    def forward(self, b_state):
        ...
        if self.seq == 'rnn':
            ...
            seq_embeddings = h.permute(1, 0, 2)  # [N x 1 x D]
        elif self.seq == 'transformer':
