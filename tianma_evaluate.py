...
from tianma_utils import *
from tianma_sum_tree import SumTree

from tianma_env_binary_question import BinaryRecommendEnv
from tianma_env_enumerated_question import EnumeratedRecommendEnv
from tianma_evaluate import tianma_evaluate
from tianma_model import TianmaAgent, TianmaReplayMemoryPER
from tianma_gcn import TianmaGraphEncoder
import time
import warnings
...
def tianma_evaluate(args, kg, dataset, filename):
    test_env = TianmaEnvDict[args.data_name](kg, dataset, args.data_name, args.embed, seed=args.seed, max_turn=args.max_turn,
    ...
    memory = TianmaReplayMemoryPER(args.memory_size)
    ...
    gcn_net = TianmaGraphEncoder(device=args.device, entity=embed.size(0), emb_size=embed.size(1), kg=kg, embeddings=embed,
                                 fix_emb=args.fix_emb, seq=args.seq, gcn=args.gcn, hidden_size=args.hidden).to(args.device)
    agent = TianmaAgent(device=args.device, memory=memory, state_size=args.hidden, action_size=embed.size(1),
    ...
    for user_num in tqdm(range(user_size)):
        blockPrint()
        ...
        if (user_num + 1) % args.observe_num == 0 and user_num > 0:
            ...
            print('Total evaluation epoch_user:{}'.format(user_num + 1))
            print('Takes {} seconds to finish {}% of this task'.format(str(time.time() - start), float(user_num) * 100 / user_size))
            print('SR5:{}, SR10:{}, SR15:{}, AvgT:{}, Rank:{} Total epoch_user:{}'.format(SR5 / args.observe_num, SR10 / args.observe_num, SR15 / args.observe_num,
    ...
    save_tianma_metric(dataset=args.data_name, filename=filename, epoch=user_num, SR=SR_all, spend_time=time.time() - start,
                       mode='test')
    save_tianma_metric(dataset=args.data_name, filename=test_filename, epoch=user_num, SR=SR_all, spend_time=time.time() - start,
                       mode='test')
    ...
    PATH = TMP_DIR[args.data_name] + '/Tianma-log-merge/' + test_filename + '.txt'
...
def main():
    ...
    parser.add_argument('--entropy_method', type=str, default='weight_entropy', help='entropy_method is one of {entropy, weight_entropy}')
    parser.add_argument('--max_turn', type=int, default=15, help='max conversation turn')
    parser.add.add_argument('--cand_len_size', type=int, default=20, help='binary state size for the length of candidate items')
    ...
    parser.add.argument('--embed', type=str, default='transe', help='pretrained embeddings')
    ...
    kg = load_tianma_kg(args.data_name)
    feature_name = TianmaFeatureDict[args.data_name]
    ...
    dataset = load_tianma_dataset(args.data_name)
    ...
    tianma_evaluate(args, kg, dataset, filename)
